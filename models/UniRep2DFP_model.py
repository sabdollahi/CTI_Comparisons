...
'''
UniRep2DFP: Feedforward Neural Network for amino acid sequences [UniRep-based Input Features] and Feedforward NN for 2D drug fingerprints
'''
class UniRep2DFP(nn.Module):
    def __init__(self):
        super(UniRep2DFP, self).__init__()
        '''[TARGET MODULE]'''
        self.tar_fc0 = nn.Linear(1900, 1024)
        self.tar_bn0 = nn.BatchNorm1d(num_features=1024)
        self.tar_drop0 = torch.nn.Dropout(0.2)
        self.tar_fc1 = nn.Linear(1024, 512)
    ...
    def forward(self, compound_input, target_input):
        ...
        target_rep_vec = self.tar_drop0(self.tar_bn0(self.tar_fc0(target_input)))
        target_rep_vec = self.tar_drop1(self.tar_bn1(self.tar_fc1(target_rep_vec)))
