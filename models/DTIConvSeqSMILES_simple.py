...
'''
    DTIConvSeqSMILES: 2D Convolutional Neural Networks using SMILES and amino acid sequences
'''
class DTIConvSeqSMILES(nn.Module):
    def __init__(self):
        super(DTIConvSeqSMILES, self).__init__()
        input_channel = 1
        output_channel = 1
        '''[TARGET MODULE]'''
        # Padding = 1; Stride = 1
        ...
        '''[COMPOUND MODULE]'''
        # (20, 1, 348, 98) ->conv-> (20, 1, 250, 60) ->maxpool-> (20, 1, 200, 40)
        self.comp_conv1 = nn.Sequential(
                        nn.Conv2d(input_channel, output_channel, kernel_size=(101,41), padding=1),
                        nn.ReLU(),
                        nn.MaxPool2d(kernel_size=(51,21), stride=1))
        # (20, 1, 200, 40) ->conv-> (20, 1, 150, 20) ->maxpool-> (20, 1, 100, 10)
        self.comp_conv2 = nn.Sequential(
                        nn.Conv2d(input_channel, output_channel, kernel_size=(53,23), padding=1),
                        nn.ReLU(),
                        nn.MaxPool2d(kernel_size=(51,11), stride=1))
        self.comp_fc1 = nn.Linear(100 * 10, 512)
    ...
    def forward(self, compound_input, target_input):
        ...
        compound_rep_vec = self.comp_conv2(compound_rep_vec)
        compound_rep_vec = self.comp_drop1(F.relu(self.comp_fc1(compound_rep_vec.view(-1, 100 * 10))))
        compound_rep_vec = self.comp_drop2(F.relu(self.comp_fc2(compound_rep_vec)))
