...
class SingleTrainTest:
    ...
    def train_test_RF(self):
        ...
        for index in range(0, len(training_index), batch_size):
            ...
            compound_input_features = compound_input_features.reshape(first_dim, smiles_maximum_length*98)
        ...
        pd.DataFrame(rf_results_data).to_csv(f'{self.main_path}/Prediction_RF.csv', index=False)
    ...
    def train_test_DTIConvSeqSMILES(self,num_of_epochs, num_of_shuffle_epochs, batch_size, learning_rate = 0.1, use_biophysicochemical_props = False, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv=''):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("DTIConvSeqSMILES: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                #Continue training on the side training set
                training_df = shuffle(side_training_df)
                training_index = training_df.index
                Y = training_df[['bind']].values
                Y = Variable(torch.LongTensor(Y.flatten()).to(torch.device(dev)), requires_grad=False)
                for epoch in range(num_of_epochs):
                    running_loss = 0
                    b = 0
                    for index in range(0, len(training_index), bsize):
                        batch_df = training_df.loc[training_index[index : index + bsize]]
                        if(len(batch_df) < 2):
                            break                            
                        batch_Y = Y[index : index + bsize]
                        targets_arr = batch_df['Sequence'].to_numpy()
                        smiles_arr = batch_df['Canonical SMILES'].to_numpy()
                        # compound_input_features dimensions: (bsize, max_smiles_length, embedding_size) in our case, max_smiles_length = 348, embedding_size = 98
                        compound_input_features = feature_preparation.generate_matrices(smiles_onehot_vectors_dict, smiles_arr, smiles_maximum_length)
                        # target_input_features dimensions: (bsize, max_target_length, embedding_size) in our case, max_target_length = 1400, embedding_size = 20
                        target_input_features = []
                        if(use_biophysicochemical_props):
                            target_input_features = feature_preparation.generate_matrices(aa_biophysicochemical_props_dict, targets_arr, targets_max_length)
                        else:
        ...
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_DTIConvSeqSMILES.csv', index=False)
        ...
        if(use_gpu):
            Evaluation_data['DTIConvSeqSMILES'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['DTIConvSeqSMILES'].append(float(accuracy_test.numpy()))
        Evaluation_data['DTIConvSeqSMILES'].append(roc_auc[1])
        Evaluation_data['DTIConvSeqSMILES'].append(PR_auc[1])
        Evaluation_data['DTIConvSeqSMILES'].append(float(num_of_ones))
        if(use_gpu):
    ...
    def train_test_DTISeq2DFP(self,num_of_epochs, num_of_shuffle_epochs, name_2D_FP, batch_size, learning_rate = 0.1, use_biophysicochemical_props = False, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv=''):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("DTISeq2DFP: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                #Continue training on the side training set
                training_df = shuffle(side_training_df)
                training_index = training_df.index
                ...
                for epoch in range(num_of_epochs):
                    ...
                    for index in range(0, len(training_index), bsize):
                        ...
                        targets_arr = batch_df['Sequence'].to_numpy()
                        smiles_arr = batch_df['Canonical SMILES'].to_numpy()
                        # compound_input_features dimensions: (bsize, 3239)
                        compound_input_features = feature_preparation.get_2D_drug_fingerprints(smiles_arr)
                        # target_input_features dimensions: (bsize, max_target_length, embedding_size) in our case, max_target_length = 1400, embedding_size = 20
                        target_input_features = []
                        if(use_biophysicochemical_props):
                    ...
                    print(f">>>>>>>>>>{str(count_epoch)}/{str(total_epochs)}>>>>>>>>>>Epoch loss: {str(running_loss/b)}")
                    count_epoch += 1
        pd.DataFrame(Loss_data).to_csv(f'{self.main_path}/Loss_DTISeq2DFP.csv', index=False)
        ...
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_DTISeq2DFP.csv', index=False)
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['DTISeq2DFP'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['DTISeq2DFP'].append(float(accuracy_test.numpy()))
        Evaluation_data['DTISeq2DFP'].append(roc_auc[1])
        Evaluation_data['DTISeq2DFP'].append(PR_auc[1])
        Evaluation_data['DTISeq2DFP'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
    ...
    def train_test_BERT2DFP(self,num_of_epochs, num_of_shuffle_epochs, name_2D_FP, batch_size, learning_rate = 0.1, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv=''):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("BERT2DFP: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                #Continue training on the side training set
                training_df = shuffle(side_training_df)
                training_index = training_df.index
        ...
        if(use_gpu):
            ...
            prediction_read_data['real'] = Y_test.numpy()
        prediction_read_data['prediction'] = numpy_pred_Y[:, 1]
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_BERT2DFP.csv', index=False)
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['BERT2DFP'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['BERT2DFP'].append(float(accuracy_test.numpy()))
        Evaluation_data['BERT2DFP'].append(roc_auc[1])
        Evaluation_data['BERT2DFP'].append(PR_auc[1])
        Evaluation_data['BERT2DFP'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
        print(f"ROC AUC: {str(roc_auc[1])}")
        print(f"PR AUC: {str(PR_auc[1])}")
        pd.DataFrame(Evaluation_data, index=['Accuracy','ROC AUC','PR AUC', 'Num of Positive Predicted']).to_csv(f'{self.main_path}/Evaluation_BERT2DFP.csv')      
    ...
    def train_test_UniRep2DFP(self,num_of_epochs, num_of_shuffle_epochs, name_2D_FP, batch_size, learning_rate = 0.1, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv=''):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("UniRep2DFP: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                #Continue training on the side training set
                training_df = shuffle(side_training_df)
                training_index = training_df.index
        ...
        if(use_gpu):
            ...
            prediction_read_data['real'] = Y_test.numpy()
        prediction_read_data['prediction'] = numpy_pred_Y[:, 1]
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_UniRep2DFP.csv', index=False)
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['UniRep2DFP'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['UniRep2DFP'].append(float(accuracy_test.numpy()))
        Evaluation_data['UniRep2DFP'].append(roc_auc[1])
        Evaluation_data['UniRep2DFP'].append(PR_auc[1])
        Evaluation_data['UniRep2DFP'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
        print(f"ROC AUC: {str(roc_auc[1])}")
        print(f"PR AUC: {str(PR_auc[1])}")
        pd.DataFrame(Evaluation_data, index=['Accuracy','ROC AUC','PR AUC', 'Num of Positive Predicted']).to_csv(f'{self.main_path}/Evaluation_UniRep2DFP.csv') 
    ...
    def train_test_DTISeqE3FP(self,num_of_epochs, num_of_shuffle_epochs, name_3D_FP, max_num_of_conformers, batch_size, learning_rate = 0.1, use_biophysicochemical_props = False, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv=''):  
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("DTISeqE3FP: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                training_df = shuffle(side_training_df)
                training_index = training_df.index
                Y = training_df[['bind']].values
                ...
                for epoch in range(num_of_epochs):
                    ...
                    for index in range(0, len(training_index), bsize):
                        ...
                        targets_arr = batch_df['Sequence'].to_numpy()
                        smiles_arr = batch_df['Canonical SMILES'].to_numpy()
                        # compound_input_features dimensions: (bsize, max_num_of_conformers, 2048)
                        compound_input_features = feature_preparation.get_3D_drug_fingerprints(smiles_arr, max_num_of_conformers)
                        # target_input_features dimensions: (bsize, max_target_length, embedding_size) in our case, max_target_length = 1400, embedding_size = 20
                        target_input_features = []
                        if(use_biophysicochemical_props):
        ...
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_DTISeqE3FP.csv', index=False)
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['DTISeqE3FP'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['DTISeqE3FP'].append(float(accuracy_test.numpy()))
        Evaluation_data['DTISeqE3FP'].append(roc_auc[1])
        Evaluation_data['DTISeqE3FP'].append(PR_auc[1])
        Evaluation_data['DTISeqE3FP'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
    ...
    def train_test_PhyChemDG(self, num_of_epochs, num_of_shuffle_epochs, batch_size = 64, learning_rate = 0.0001, use_biophysicochemical_props = False, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv='', recompute_max_num_atoms = False, drug_ids_adr = '../Datasets/Conformers/ConformersSetIds-Drugs.csv', drug_graphs_and_features_adr = '../Datasets/Drugs_2D_Graphs_AtomFeatures'):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("PhyChemDG: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                training_df = shuffle(side_training_df)
                training_index = training_df.index
                Y = training_df[['bind']].values
                ...
                for epoch in range(num_of_epochs):
                    ...
                    for index in range(0, len(training_index), bsize):
                        ...
                        for target in targets_arr:
                            all_tar_num_aa.append(len(target))
                        # The input features of the drugs 
                        # compounds_adjacency_matrices dimesnions: (bsize, max_num_atoms, max_num_atoms) in our case, max_num_atoms = 148
                        # compounds_atom_features dimensions: (bsize, max_num_atoms, num_of_atom_features) in our case, num_of_atom_features = 34 and max_num_atoms = 148
                        compounds_adjacency_matrices, compounds_atom_features, all_cpds_num_atoms = feature_preparation.get_2Dgraphs_atom_features(smiles_arr, max_num_atoms, device)         
                        # target_input_features dimensions: (bsize, max_target_length, embedding_size) in our case, max_target_length = 1400, embedding_size = 20
                        target_input_features = []
                        if(use_biophysicochemical_props):
                            ...
                            target_input_features = feature_preparation.generate_matrices(aa_onehot_vectors_dict, targets_arr, targets_max_length)
                        # target_input_features dimensions: (bsize, 1, max_target_length, embedding_size)
                        # target_input_features = target_input_features.reshape((target_input_features.shape[0], 1, targets_max_length, aa_embedding_length))
                        target_input_features = torch.FloatTensor(target_input_features).to(torch.device(dev))
                        batch_features = (compounds_atom_features, compounds_adjacency_matrices, target_input_features, labels_new, all_cpds_num_atoms, all_tar_num_aa)
                        loss_train = trainer.train(batch_features, device)
        ...
        pd.DataFrame(prediction_real_data).to_csv(f'{self.main_path}/Prediction_PhyChemDG.csv', index=False)
    ...
    def train_test_GrAttCPI(self,num_of_epochs, num_of_shuffle_epochs, batch_size, learning_rate = 0.1, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv='', drug_ids_adr = '../Datasets/Conformers/ConformersSetIds-Drugs.csv'):  
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("GrAttCPI: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                training_df = shuffle(side_training_df)
                if(len(training_df)%bsize == 1):
                    training_df = training_df.head(len(training_df)-1)
                training_index = training_df.index
                Y = training_df[['bind']].values
                Y = Variable(torch.LongTensor(Y.flatten()).to(torch.device(dev)), requires_grad=False)
                for epoch in range(num_of_epochs):
                    ...
                    print(f">>>>>>>>>>{str(count_epoch)}/{str(total_epochs)}>>>>>>>>>>Epoch loss: {str(running_loss/b)}")
                    count_epoch += 1
        pd.DataFrame(Loss_data).to_csv(f'{self.main_path}/Loss_GrAttCPI.csv', index=False)
        ...
        if(use_gpu):
            ...
            prediction_read_data['real'] = Y_test.numpy()
        prediction_read_data['prediction'] = numpy_pred_Y[:, 1]
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_GrAttCPI.csv', index=False)    
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['GrAttCPI'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['GrAttCPI'].append(float(accuracy_test.numpy()))
        Evaluation_data['GrAttCPI'].append(roc_auc[1])
        Evaluation_data['GrAttCPI'].append(PR_auc[1])
        Evaluation_data['GrAttCPI'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
    ...
    def train_test_PhyGrAtt(self,num_of_epochs, num_of_shuffle_epochs, batch_size, learning_rate = 0.1, use_gpu = False, gpu_device = 0, is_side_training=False, side_training_csv='', drug_ids_adr = '../Datasets/Conformers/ConformersSetIds-Drugs.csv'):
        ...
        if(is_side_training):
            bsize = 32
            count_epoch = 0
            print("PhyGrAtt: Side-Training the model has started!")
            for shuffled_epoch in range(num_of_shuffle_epochs):
                training_df = shuffle(side_training_df)
                if(len(training_df)%bsize == 1):
                    training_df = training_df.head(len(training_df)-1)
                ...
                for epoch in range(num_of_epochs):
                    ...
                    print(f">>>>>>>>>>{str(count_epoch)}/{str(total_epochs)}>>>>>>>>>>Epoch loss: {str(running_loss/b)}")
                    count_epoch += 1
        pd.DataFrame(Loss_data).to_csv(f'{self.main_path}/Loss_PhyGrAtt.csv', index=False)
        ...
        if(use_gpu):
            ...
            prediction_read_data['real'] = Y_test.numpy()
        prediction_read_data['prediction'] = numpy_pred_Y[:, 1]
        pd.DataFrame(prediction_read_data).to_csv(f'{self.main_path}/Prediction_PhyGrAtt.csv', index=False)    
        accuracy_test = (concatenated_pred_Y == Y_test).long().sum().float() /  concatenated_pred_Y.size()[0]    
        Y_real = np.array([[1,0] if y == 0 else [0,1] for y in Y_test])
        fpr = dict()
        tpr = dict()
        ...
        for i in range(2):
            ...
            PR_auc[i] = auc(recall[i], precision[i])
        if(use_gpu):
            Evaluation_data['PhyGrAtt'].append(float(accuracy_test.cpu().numpy()))
        else:            
            Evaluation_data['PhyGrAtt'].append(float(accuracy_test.numpy()))
        Evaluation_data['PhyGrAtt'].append(roc_auc[1])
        Evaluation_data['PhyGrAtt'].append(PR_auc[1])
        Evaluation_data['PhyGrAtt'].append(float(num_of_ones))
        if(use_gpu):
            print(f"Accuracy: {str(float(accuracy_test.cpu().numpy()))}")
        else:
            print(f"Accuracy: {str(float(accuracy_test.numpy()))}")
